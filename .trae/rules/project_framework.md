...
- 当前线上 Supabase 管理员 SQL 仍存在版本差：旧环境缺少 `feedback_messages.is_read/read_at` 字段以及删除/批量删除/批量已读 RPC，因此前端已先补旧接口兼容、以前端完整反馈列表重算统计口径，并把删除报错改成清晰提示；真正删除能力仍依赖重新执行最新版 `supabase/admin_dashboard.sql`。
- 当前正式平台已能看到新版管理员后台，但建房自动推荐仍存在多个人数下摘要与折叠详情不同步的问题，已定位为下一轮优先修复项。
