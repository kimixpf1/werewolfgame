...
- 全面修复各人数下自动推荐摘要与折叠角色配置不一致的 Bug。
- 重新构建 dist 并确认构建产物包含最新代码，推送部署到线上。
...
- 继续核查建房自动推荐逻辑，全面修复各人数下推荐摘要与折叠角色配置不一致的问题。
...
- 用户已在正式平台确认管理员后台已切到新版，说明“玩家统计”和“建议信箱”两个独立页面及批量操作入口的线上静态包已经生效。
- 已收到新的建房逻辑反馈：不只 14 人，15 人以及其他人数下也存在自动推荐时顶部摘要与下方折叠角色配置不一致的情况，说明当前推荐结果和展示详情之间仍存在系统性的同步缺口。
...
- 全面修复各人数下自动推荐摘要与折叠角色配置不一致的问题，并补做多人数建房真人页面验收。
